...
// @desc    Generate images using Google Gemini AI
// @access  Private
router.post('/generate-with-gemini', auth, [
  ...
  try {
    ...
    console.log('Generating images with Gemini AI:', { prompt, style, quality, aspectRatio, numberOfImages });
    ...
    for (const imageData of result.images) {
      try {
        const image = new Image({
          ...
          style,
          quality,
          generationModel: 'gemini-imagen',
          generationService: 'google-gemini',
    ...
    res.json({
      message: 'Images generated successfully with Gemini AI',
...
router.post('/generate-website-images-gemini', auth, [
  ...
  try {
    ...
    console.log('Generating website images with Gemini AI:', { businessName, industry, style });

    // Generate website images using Gemini AI
    const result = await geminiImageService.generateWebsiteImages({
      businessName,
      industry,
      style,
    ...
    res.json({
      message: 'Website images generated successfully with Gemini AI',
      images: result.images,
      savedImages,
      metadata: {
        businessName,
    ...
    console.error('Error generating website images with Gemini:', error);
    res.status(500).json({
      message: 'Failed to generate website images',
      error: error.message
    });
